...
def print_msssim_score(fake_image_torch, batch=100):
  fake_image_list = fake_image_torch.transpose((0,2,3,1))
  ms_num_image = fake_image_list.size(0)

  all_sims = []
  for i1 in range(int(ms_num_image / batch)):
    for i2 in range(int(ms_num_image / batch)):
      if i1 == i2:
        continue
      for _ in range(int(ms_num_image / batch)):
        sim = MultiScaleSSIM((fake_image_list[i1 * batch:(i1 + 1) * batch]),
                             (fake_image_list[i2 * batch:(i2 + 1) * batch]),
                             max_val=255)
      print((i1 + 1), '/', int(ms_num_image / batch), ' ', '-', i2, ' done')
      if not np.isnan(sim):
        all_sims.append(sim)
  print("mean msssim score is : %.4f, std of msssim score is : %.4f" % (np.mean(all_sims), np.std(all_sims)))
  return 0
